            ...
            if let error = error {
                ...
                if querySnapshot.documents.isEmpty || querySnapshot.documents.count > 1 {
                    ...
                    if querySnapshot.documents[0].documentID == Auth.auth().currentUser!.uid {
                        ...
                        let pendingReq: [String] = data["pendingFriendReq"]! as! [String]
                        let friendReq: [String] = data["friendReq"]! as! [String]
                        let friends: [String] = data["friends"]! as! [String]
                        
                        if pendingReq.contains(currentUserID) {
                            completion(nil, .alreadyRecievedRequest)
                        } else if friendReq.contains(currentUserID) {
                            completion(nil, .alreadySentRequest)
                        } else if friends.contains(currentUserID) {
                            completion(nil, .alreadyFriends)
                        } else {
                            let displayUsername = data["displayUsername"]! as! String
                            let userID = querySnapshot.documents[0].documentID
                            completion(BasicUser(displayUsename: displayUsername, username: self.username, userID: userID), nil)
                        }
